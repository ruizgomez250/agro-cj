...
export default function Edit({ contratista }) {
    const { data, setData, put, processing, errors } = useForm({
        nombre: contratista.nombre || '',
        cedula: contratista.cedula || '',
        telefono: contratista.telefono || '',
        especialidad: contratista.especialidad || '',
        tarifa_hora: contratista.tarifa_hora || '',
        saldo_actual: contratista.saldo_actual || '',
        observaciones: contratista.observaciones || '',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('contratistas.update', contratista.id));
    };
    ...
    return (
        <AuthenticatedLayout
            header={
                <div className="flex items-center justify-between">
                    <h2 className="text-xl font-semibold text-gray-800">Editar Contratista</h2>
                    <Link
        ...
        >
            <Head title={`Editar - ${contratista.nombre}`} />
