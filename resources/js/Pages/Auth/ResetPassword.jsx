...
export default function ResetPassword({ token, email }) {
    ...
    const labelStyle = { display: 'block', fontSize: '0.8rem', fontWeight: 500, color: 'rgba(255,255,255,0.75)', marginBottom: '0.45rem' };
    ...
    return (
        <GuestLayout>
            ...
            <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
                ...
                <h2 style={{ marginTop: '1rem', fontSize: '1.35rem', fontWeight: 700, color: '#fff' }}>Nueva Contrasena</h2>
                <p style={{ fontSize: '0.82rem', color: 'rgba(255,255,255,0.55)', marginTop: '0.35rem' }}>Ingrese su nueva contrasena</p>
            </div>
            <form onSubmit={submit}>
                <div style={{ marginBottom: '1.15rem' }}>
                    <label htmlFor="email" style={labelStyle}>Correo Electronico</label>
                    <input id="email" type="email" name="email" value={data.email} onChange={(e) => setData('email', e.target.value)} autoComplete="username" placeholder="correo@ejemplo.com" style={inputStyle} onFocus={handleFocus} onBlur={handleBlur} />
                    <InputError message={errors.email} style={{ color: '#f87171', fontSize: '0.78rem', marginTop: '0.4rem' }} />
                </div>
                <div style={{ marginBottom: '1.15rem' }}>
                    <label htmlFor="password" style={labelStyle}>Nueva Contrasena</label>
                    <input id="password" type="password" name="password" value={data.password} onChange={(e) => setData('password', e.target.value)} autoComplete="new-password" autoFocus placeholder="Minimo 8 caracteres" style={inputStyle} onFocus={handleFocus} onBlur={handleBlur} />
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                    ...
                    <input id="password_confirmation" type="password" name="password_confirmation" value={data.password_confirmation} onChange={(e) => setData('password_confirmation', e.target.value)} autoComplete="new-password" placeholder="Repita la contrasena" style={inputStyle} onFocus={handleFocus} onBlur={handleBlur} />
                ...
                <button type="submit" disabled={processing} style={{ width: '100%', padding: '0.8rem', border: 'none', borderRadius: '14px', background: 'linear-gradient(135deg, #065F46 0%, #059669 50%, #10B981 100%)', color: '#fff', fontSize: '0.9rem', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', cursor: processing ? 'not-allowed' : 'pointer', opacity: processing ? 0.7 : 1, boxShadow: '0 4px 24px rgba(6,95,70,0.45), inset 0 1px 0 rgba(255,255,255,0.15)', transition: 'all 0.3s ease' }}>
                    {processing ? 'Restableciendo...' : 'Restablecer Contrasena'}
                </button>
            </form>
        </GuestLayout>
    );
