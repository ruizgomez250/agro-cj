...
export default function Edit({ recepcion, proveedores }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'put',
        fecha: recepcion.fecha || '',
        proveedor_id: recepcion.proveedor_id || '',
        guia_remision: recepcion.guia_remision || '',
        cantidad_galones: recepcion.cantidad_galones || '',
        precio_galon: recepcion.precio_galon || '',
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('gasoil.recepcion.update', recepcion.id), { forceFormData: true });
    };
    ...
    return (
        <AuthenticatedLayout
            header={
                <div>
                    <h2 className="text-xl font-semibold text-gray-800">Editar Recepcion</h2>
                    <p className="text-sm text-gray-500 mt-0.5">Modificar recepcion #{recepcion.id}</p>
                </div>
            }
        >
            <Head title={`Editar Recepcion #${recepcion.id}`} />
                    ...
                    <div className="bg-white rounded-xl shadow-md overflow-hidden">
                        ...
                        <form onSubmit={submit} className="p-6 space-y-6">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700">Comprobante {recepcion.comprobante_url && '(actual adjunto)'}</label>
                            ...
                            <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100">
                                ...
                                >
                                    {processing ? 'Actualizando...' : 'Actualizar Recepcion'}
                                </button>
                            </div>
                        </form>
                    </div>
